...
function AdminDashboard({ projects, userName, onNavigate, terminatedProjectsInit = [], cmmMode = false, forcePanel = null, onForcePanelClose }: { projects: Project[]; userName: string; onNavigate?: (mode: string) => void; terminatedProjectsInit?: Project[]; cmmMode?: boolean; forcePanel?: ForcedPanel | null; onForcePanelClose?: () => void }) {
  ...
  // Recherche LOCALE aux panneaux "RDV … à fixer" : filtre uniquement les
  // projets déjà listés dans le panneau ouvert (ne va JAMAIS chercher ailleurs).
  ...
  return (
    <div className="mb-6 space-y-4">
      ...
      {showSummaryPanel && (() => {
        ...
        const rdvSearchable = !!rdvStatusFieldFn;
        const panelQuery = rdvSearchable ? panelSearch.trim() : "";
        if (panelQuery) {
          const terms = stripAccents(panelQuery.toLowerCase()).split(/\s+/).filter(Boolean);
          panelProjects = panelProjects.filter((p) => {
            const hay = stripAccents(
              [
                p.projet, p.ofrTM, p.nomChantier, p.adresseChantier,
                p.cmdTM, p.cmdTMUsine, p.cmdGrossiste, p.ofrGrossiste,
                p.cmdFournisseurs, p.servCmdFournisseurs, p.servMesuresFournisseurs,
            ...
            <div className="flex items-center justify-between gap-2 mb-2 flex-wrap">
              ...
              <div className="flex items-center gap-2 flex-wrap justify-end">
                {/* Recherche LOCALE : filtre uniquement les projets du panneau
                    ouvert (RDV … à fixer). Ne cherche jamais ailleurs. */}
                {rdvSearchable && (
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) => setPanelSearch(e.target.value)}
                      placeholder="Rechercher dans la liste…"
                      className="text-xs pl-7 pr-7 py-1.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-slate-700 text-gray-700 dark:text-gray-200 w-44 sm:w-52 focus:outline-none focus:ring-2 focus:ring-blue-400/50"
                    />
                    {panelSearch && (
                      <button
                        type="button"
                        onClick={() => setPanelSearch("")}
                        className="absolute right-1.5 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center rounded-full text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-600"
                        title="Effacer"
                      >
                        <X className="w-3 h-3" />
                      </button>
            ...
            {panelProjects.length === 0 && (
              panelQuery ? (
                <p className="text-sm text-gray-400 py-2">
                  Aucun projet ne correspond à « {panelQuery} » dans cette liste.
                </p>
